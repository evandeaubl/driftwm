...
- `opacity` — 0.0–1.0: window transparency (default: 1.0, fully opaque). Below 1.0 a fullscreen window shows the canvas behind it instead of black: the wallpaper and canvas layers keep drawing under it, though panels and other windows stay hidden. Costs that output its direct scan-out for as long as the window is fullscreen, so leave games opaque.
